...
- `create_engine("mysql+pymysql://", connect_args={...})` 可用于规避 URL 中密码特殊字符问题；真实连接参数不得写入 skill 或 GitHub。
- 入库函数应先读取 `SHOW COLUMNS`，只写数据库真实存在字段，忽略 `序号`、导入时间等自动字段。
- upsert SQL 应明确唯一键列和更新列；唯一键缺失、全空或清洗后为空时跳过或报错，不做静默插入。
- 按期间/公司覆盖的结果表，删除范围必须来自本次数据实际期间和公司组合，不能误删全表。
- 覆盖更新前必须确认业务粒度、删除键、DataFrame 去重键、数据库唯一索引和界面 `key_columns` 一致；关键字段缺列或为空时必须报错，不能退回更宽范围删除。
- 删除旧数据和插入新数据必须在同一个可回滚事务中；事务内禁止穿插 `ALTER TABLE`、`TRUNCATE`、`DROP TABLE`、`CREATE TABLE` 等会隐式提交的 DDL。
